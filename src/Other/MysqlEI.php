...
class MysqlEI
{
    ...
    public function export($tablename = '', $dir = '', $prefix = '', $ignoreData = [], $name = '', $size = 1024)
    {
        ...
        $thanSize = function (&$_sql, &$p, $filename) use ($size, $dir, &$msg) {
            if (strlen($_sql) >= $size * 1024) {
                ...
                if ($res === true) {
                    $volume = $p === 1 ? '' : 'volume-' . $p . ' ';
                    echo self::getColoredString("{$volume}backup completed.", 'light_green') . PHP_EOL;
                    echo self::getColoredString("Generate a backup file {$file}.", 'cyan') . PHP_EOL;
                } else {
                    Z::finish(self::getColoredString('Exception', "volume-" . $p . " backup error:" . $res, 'red'));
                }
                $p++;
                $_sql = "";
            }
        ...
        if ($dir && $sql != "") {
            ...
            if ($res === true) {
                $volume = $p === 1 ? '' : 'volume-' . $p . ' ';
                echo self::getColoredString("{$volume}backup completed.", 'light_green') . PHP_EOL;
                echo self::getColoredString("Generate a backup file {$filename}.", 'cyan') . PHP_EOL;
            } else {
    ...
    function import($sqlfile, $tablePrefix = null)
    {
        ...
        echo self::getColoredString("Import backup data") . PHP_EOL;
        if (empty ($volume [1])) {
            echo self::getColoredString('Import sql：' . $sqlfile) . PHP_EOL;
            z::throwIf(!$this->_import($sqlfile, $tablePrefix), 'Exception', 'Database import failed');
            echo self::getColoredString('Database import successful') . PHP_EOL;
        } else {
            ...
            while ($volume_id) {
                ...
                if (file_exists($tmpfile)) {
                    echo self::getColoredString("Importing sub-volumes{$volume_id}: {$tmpfile}") . PHP_EOL;
                    ...
                    echo self::getColoredString("This backup of all volumes was successfully imported") . PHP_EOL;
                    break;
                }
                $volume_id++;
            }
